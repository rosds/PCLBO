...
template <typename PointT> class HeatKernelSignature {
public:
  ...
  void setEigenValues(const Eigen::VectorXd &values) { _eigenvalue = values; }

  void setEigenFunctions(const Eigen::MatrixXd &vectors) {
    _eigenfunctions = vectors;
  }

  HeatSignature compute(const int x, const double t,
                        const int n_functions = 200);
