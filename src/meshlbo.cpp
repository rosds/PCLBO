...
inline double cot(const double angle) { return 1.0 / tan(angle); }

inline double angleBetween(const Eigen::Vector3f &a, const Eigen::Vector3f &b) {
  Eigen::Vector3f c = a.cross(b);
  return atan2f(c.norm(), a.dot(b));
}

void pclbo::MeshLBOEstimation::compute() {
